"""
Floating Point Issue Queue Diagnostic Generator

Generates diagnostic programs that stress the floating-point issue queue
by creating varying numbers of dependent FP operations.

The generated code uses pointer chasing with additional floating-point
operations to fill the FP issue queue to different levels.
"""
...
# Base assembly template for FP issue queue stress
# Uses pointer chasing with FP operations to stress the FP issue queue
base = """\
    asm volatile(
...
def filler(num_ops):
    ...
    asm volatile(".endr");
    asm volatile("movq %xmm3, %r12"); // trigger
"""    
...
def generator(args):
    ...
    bin_name = f"{bin_dir}/fp_isq.{num_ops}.diag"
