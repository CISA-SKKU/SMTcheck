"""
Integer Issue Queue Diagnostic Generator

Generates diagnostic programs that stress the integer issue queue
by creating varying numbers of dependent integer ALU operations.

The generated code uses pointer chasing with additional integer
operations to fill the integer issue queue to different levels.
"""
...
# Base assembly template for integer issue queue stress
# Uses pointer chasing with integer ALU ops to stress the issue queue
base = """\
    asm volatile(
...
def filler(num_ops):
    ...
    asm volatile("addq %r15, %r15");
    asm volatile(".endr");
    asm volatile("imul %r8, %r12"); // trigger
"""
...
def generator(args):
    ...
    bin_name = f"{bin_dir}/int_isq.{num_ops}.diag"
