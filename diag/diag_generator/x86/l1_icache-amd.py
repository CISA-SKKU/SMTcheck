"""
L1 Instruction Cache Diagnostic Generator (AMD)

Generates diagnostic programs that stress the L1 instruction cache by
creating code that spans different cache lines and sets.

The generated assembly uses jump chains across aligned code blocks
to force instruction fetches from different cache locations.

Parameters varied:
    - window_size: Code block alignment (determines cache line usage)
...
"""
    ...
    code_name = f"{code_gen_dir}/l1_icache-amd.{window_size}.{num_entries}.s"
    bin_name = f"{bin_dir}/l1_icache-amd.{window_size}.{num_entries}.diag"
    template_file = f"templates/l1_icache-amd.cpp"
