"""
Load-Store Queue (LSQ) Diagnostic Generator

Generates diagnostic programs that stress the load-store queue by
...
that cannot complete until the primary load resolves, thus occupying
LSQ entries.
"""
...
# Base assembly template for load-store queue stress
# Uses pointer chasing with dependent loads to fill the LSQ
base = """\
    asm volatile(
...
def generator(args):
    ...
    bin_name = f"{bin_dir}/load_lsq.{num_ops}.diag"
