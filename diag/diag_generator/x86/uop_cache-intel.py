"""
Micro-op Cache Diagnostic Generator (Intel)

Generates diagnostic programs that stress the micro-op (uop) cache by
creating code patterns that vary in uop cache footprint.

The generated assembly uses jump chains with NOP sleds to create
code that occupies different amounts of uop cache entries.

Parameters varied:
    - window_size: Code block alignment (determines indexing granularity)
    - num_entries: Number of code blocks (determines total footprint)

This Intel-specific version uses instruction patterns optimized for
Intel's uop cache architecture.
"""
...
"""
    ...
    code_name = f"{code_gen_dir}/uop_cache-intel.{window_size}.{num_entries}.s"
    bin_name = f"{bin_dir}/uop_cache-intel.{window_size}.{num_entries}.diag"
    template_file = f"templates/uop_cache-intel.cpp"
...
if __name__ == "__main__":
    entire_working_set_size = (1<<14) # 16KB
    min_num_entries = 8

    parameter_list = []

    sample_point_per_window_size = 32
    for i in range(10):
