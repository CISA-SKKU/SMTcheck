...
class OutputStruct:
    ...
    def __init__(self, is_smt, stride, num_ways, IPC, L2_MISS_RATE):
        self.is_smt = is_smt
        self.stride = stride
...
def split_dir(dir):
    ...
    return OutputStruct(is_smt=is_smt, stride=stride, num_ways=num_ways, IPC=extract_IPC(dir), L2_MISS_RATE=extract_miss_rate(dir))
...
def parse(dir, feature_name):
    ...
    for output in outputs:
        plot_dict[output.is_smt][output.stride].append((output.num_ways, output.IPC, output.L2_MISS_RATE))
