...
def extract_IPC(dir):
    ...
    for line in lines:
        line = line.split(": ")
        if line[0] == "ic_access_per_branch":
            return float(line[1])
...
def parse(dir, feature_name):
    ...
    for is_smt, IPC_dict in plot_dict.items():
        ...
        for window_size, IPC_list in IPC_dict.items():
            ...
            plt.plot(x_values, y_values, label=window_size, marker="o")
        
        plt.legend(title="Window_size(Byte)")
        plt.grid(True)
        plt.title(f"{feature_name} Diag Results - {is_smt}")
        plt.xlabel("Num Entries")
        plt.ylabel("Uop Cache Cache Miss Rate")
