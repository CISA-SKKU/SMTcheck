"""
Floating-Point Issue Queue (FP ISQ) Diagnostic Runner

Runs the floating-point issue queue diagnostics and measures IPC.
The diagnostics fill the FP issue queue with varying numbers of
operations to characterize queue contention behavior.

With SMT enabled, a dummy process runs on the sibling thread
to create contention for shared issue queue resources.

Usage:
    python fp_isq.py <bin_dir> <output_dir> <core_id>[,<sibling_core_id>]
...
if __name__ == "__main__":
    ...
    # Sort diagnostics by operation count (extracted from filename: diag.N.diag)
    diag_list = sorted(
