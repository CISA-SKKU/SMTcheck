"""
L1 Data Cache Diagnostic Runner

Runs the L1 data cache diagnostics to measure cache contention behavior.
This script tests different stride sizes and associativity (ways) to
characterize how the cache responds to different access patterns.
...
Usage:
    python l1_dcache-intel.py <bin_dir> <output_dir> <core_id>[,<sibling_core_id>]
...
if __name__ == "__main__":
    ...
    if not check_hugepages():
        ...
        print("=" * 70)
        print("\nL1 data cache diagnostics require 1GB hugepages to eliminate")
    ...
    # Configuration parameters for L1 cache testing
    max_ways = 32          # Maximum associativity to test
    use_hugepage = 1       # Enable hugepage usage
    num_sets = 1           # Number of cache sets to target
    start, end = 6, 15     # Stride range: 2^6 (64B) to 2^15 (32KB)
    ...
    for stride_log2 in range(start, end+1):
        ...
        for num_ways in range(1, max_ways + 1):
            ...
            # Run diagnostic for 2 seconds, then interrupt to collect counters
            result = subprocess.run(
                ["timeout", "-s", "SIGINT", "2s", "taskset", "-c", coreids[0], diag, 
