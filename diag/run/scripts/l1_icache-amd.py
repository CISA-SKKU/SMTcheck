"""
L1 Instruction Cache Diagnostic Runner

Runs the L1 ICache diagnostics and measures IPC.
The diagnostics fill the L1 ICache with varying numbers of entries
to characterize cache contention behavior.

With SMT enabled, a dummy process runs on the sibling thread
to create contention for shared L1 ICache resources.

Usage:
    python l1_icache-amd.py <bin_dir> <output_dir> <core_id>[,<sibling_core_id>]
