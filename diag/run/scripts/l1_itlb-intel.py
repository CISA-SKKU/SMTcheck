"""
L1 Instruction TLB (L1 ITLB) Diagnostic Runner

Runs the L1 instruction TLB diagnostics and measures performance.
The diagnostics access varying numbers of pages to characterize 
L1 ITLB capacity and contention behavior.

With SMT enabled, a dummy process runs on the sibling thread
to create contention for shared L1 ITLB resources.

Usage:
    python l1_itlb-intel.py <bin_dir> <output_dir> <core_id>[,<sibling_core_id>]
...
if __name__ == "__main__":
    ...
    # Sort diagnostics by page count (extracted from filename: diag.N.diag)
    diag_list = sorted(
