"""
Load-Store Queue (LSQ) Diagnostic Runner

Runs the load-store queue diagnostics and measures IPC.
The diagnostics fill the LSQ with varying numbers of memory operations
to characterize queue contention behavior.

With SMT enabled, a dummy process runs on the sibling thread
to create contention for shared LSQ resources.

Usage:
    python load_lsq.py <bin_dir> <output_dir> <core_id>[,<sibling_core_id>]
