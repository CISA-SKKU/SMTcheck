"""
Reorder Buffer (ROB) Diagnostic Runner

Runs the reorder buffer diagnostics and measures IPC.
The diagnostics fill the ROB with varying numbers of in-flight
operations to characterize buffer contention behavior.

With SMT enabled, a dummy process runs on the sibling thread
to create contention for shared ROB resources.

Usage:
    python rob.py <bin_dir> <output_dir> <core_id>[,<sibling_core_id>]
