"""
Integer Issue Queue Injector Generator

Generates injector programs that stress the integer issue queue by
creating varying numbers of integer ALU operations that occupy queue slots.

The generated code uses dependent add operations to prevent early retirement
and fill the issue queue to specified pressure levels.
"""
...
# Assembly template for integer issue queue stress test
# Uses a pointer chasing load followed by dependent integer operations
base = """\
    asm volatile(
...
def filler(num_ops, num_nops):
    """
    Generate filling instructions to occupy integer issue queue slots.
    
    Args:
        num_ops: Number of integer add operations to insert
        num_nops: Number of NOP-equivalent xor operations to fill ROB
...
def generator(args):
    ...
    code_name = f"{code_gen_dir}/int_isq.{num_ops}.cpp"
    bin_name = f"{bin_dir}/int_isq.{num_ops}.injector"
